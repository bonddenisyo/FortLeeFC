...
const S = `id, event_id AS "eventId", user_id AS "userId", status, guests, question_answer AS "questionAnswer", created_at AS "createdAt", cancelled_at AS "cancelledAt", promoted_at AS "promotedAt"`;
...
async function registerUser({ email, password, firstName, lastName }) {
  ...
  const role = ADMIN_EMAILS.has(email.toLowerCase()) ? 'host' : 'user';
  const userId = uid();
  await pool.query(
...
async function loginUser({ email, password }) {
  ...
  if (ADMIN_EMAILS.has(raw.email) && raw.role !== 'host') {
    await pool.query(`UPDATE users SET role = 'host' WHERE id = $1`, [raw.id]);
  }
...
async function cancelEvent(eventId, hostId) {
  ...
  if (rows[0].hostId !== hostId) throw httpError(403, 'Only the host can cancel this game');
  await pool.query(
...
module.exports = {
  ...
  listEvents, getEvent, createEvent, cancelEvent,
