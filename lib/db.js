...
async function listComments(eventId) {
  const { rows } = await pool.query(
    `SELECT c.id, c.event_id AS "eventId", c.user_id AS "userId", c.body,
            c.created_at AS "createdAt", u.name AS "userName", u.picture AS "userPicture"
     FROM comments c
...
async function addComment(eventId, userId, body) {
  const id = uid();
  await pool.query(
    `INSERT INTO comments (id, event_id, user_id, body, created_at) VALUES ($1,$2,$3,$4,$5)`,
    [id, eventId, userId, body, Date.now()]
  );
  const { rows } = await pool.query(
    `SELECT c.id, c.event_id AS "eventId", c.user_id AS "userId", c.body,
            c.created_at AS "createdAt", u.name AS "userName", u.picture AS "userPicture"
...
async function deleteComment(commentId) {
  const { rows } = await pool.query(`DELETE FROM comments WHERE id = $1 RETURNING id`, [commentId]);
  if (!rows.length) throw httpError(404, 'Comment not found');
}

async function cleanupOldEvents() {
  const { rows } = await pool.query(
    `SELECT id FROM events
     WHERE (date::date < CURRENT_DATE OR status = 'cancelled')
       AND created_at < (EXTRACT(EPOCH FROM NOW() - INTERVAL '2 months') * 1000)::BIGINT`
  );
  if (!rows.length) return;
  const ids = rows.map(r => r.id);
  await pool.query(`DELETE FROM comments WHERE event_id = ANY($1::text[])`, [ids]);
  await pool.query(`DELETE FROM signups WHERE event_id = ANY($1::text[])`, [ids]);
  await pool.query(`DELETE FROM events WHERE id = ANY($1::text[])`, [ids]);
  console.log(`Cleaned up ${ids.length} stale event(s)`);
}

module.exports = {
  init, cleanupOldEvents,
  registerUser, loginUser, getUser, listUsers, listUserEmails, updateUser, setUserRole,
